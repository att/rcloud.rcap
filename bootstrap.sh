...
# RCloud dependencies
  ## Ubuntu 14.04
  apt-get install -y -qq gcc g++ gfortran libcairo-dev libreadline-dev libxt-dev libjpeg-dev \
  libicu-dev libssl-dev libcurl4-openssl-dev subversion git automake make libtool \
  libtiff-dev gettext redis-server rsync

  # Install pandoc (or RStudio Server)
...
# Install RStudio Server (for dev)
fname="rstudio-server-0.99.485-amd64.deb"
apt-get -y -q install gdebi-core wget
...
# Set CRAN mirror for installing R packages

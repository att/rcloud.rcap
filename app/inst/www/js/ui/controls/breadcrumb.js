        ...
        var renderControl = function(control, publishEvent) {
            ...
            if (control.currentPageID) {
                ...
                if (publishEvent) {
                    ...
                    var templateStr = '<div style="color:<%=controlProperties[2].value%>" class="rcap-breadcrumb"><% if(controlProperties[0].value) { %><span class="prefix"><%=controlProperties[0].value%></span><% } %><% _.each(items, function(i){ %><% if(i.id === items[items.length - 1].id) { %><span style="color:<%=controlProperties[3].value%>"><%= i.navigationTitle %></span><% } else { %><a style="color:<%=controlProperties[1].value%>" href="javascript:void(0)" data-pageid="<%=i.id%>" data-href="<%=i.navigationTitle%>"><%=i.navigationTitle%></a><span style="color:<%=controlProperties[2].value%>" class="separator">></span><% } %><% }); %></div>';

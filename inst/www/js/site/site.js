  ...
  var Site = Class.extend({
    ...
    updatePage: function (pageObj) {
      ...
      _.each(pageObj.styleProperties, function (index, prop) {
        _.findWhere(pageObj.styleProperties, {
          uid: prop.uid
        }).value = prop.value;
      });
    ...
    canDeletePage: function (pageId) {
      var page = _.findWhere(this.pages, {
        id: pageId
      });
      ...
      if (page.depth > 1) {
        ...
        return _.filter(this.pages, function (p) {
          return p.depth === 1;
        }).length > 1;
      }
    },
    ...
    duplicatePage: function (rootPageId) {
      ...
      _.each(pagesToCopy, function (pageToCopy) {
        ...
        if (pageToCopy.id !== rootPageId) { // root page should stay the same since it will have the same parent!
          var foundMapping = _.findWhere(mappings, {
            oldId: newPage.parentId
          });
    ...
    movePage: function (pageMoveData) {
      ...
      if (pageMoveData.position === 'before') {
        ...
        position = this.pages.findIndex(function (page) {
          return page.id === pageMoveData.targetPage;
        });
    ...
    //
    //
    updateProfileVariables: function (profileVariables) {
